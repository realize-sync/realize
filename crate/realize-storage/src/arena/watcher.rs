...
enum FsEvent {
    ...
    /// Catchup couldn't find the path anymore.
    Gone(PathBuf),

    /// Catchup thinks the file at this path may have changed since it
    /// was indexed.
    NeedsUpdate(PathBuf),
}
...
impl RealWatcherWorker {
    ...
    async fn handle_event(&self, ev: &FsEvent) -> anyhow::Result<()> {
        match ev {
