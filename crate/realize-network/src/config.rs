        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for ByteValueVisitor {
            ...
            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("a number or string with byte units (e.g., 1000, \"1K\", \"1MB\")")
            }
            ...
            where
                E: de::Error,
            {
                let bytes = ByteValue::parse(value)
...
mod tests {
    ...
    fn test_byte_value_parse_with_units() {
        ...
        assert_eq!(ByteValue::parse("1t").unwrap(), 1024 * 1024 * 1024 * 1024);
        assert_eq!(ByteValue::parse("2T").unwrap(), 2 * 1024 * 1024 * 1024 * 1024);
        
        // Test TB (terabytes)
        assert_eq!(ByteValue::parse("1TB").unwrap(), 1024 * 1024 * 1024 * 1024);
        assert_eq!(ByteValue::parse("1tb").unwrap(), 1024 * 1024 * 1024 * 1024);
        assert_eq!(ByteValue::parse("4TB").unwrap(), 4 * 1024 * 1024 * 1024 * 1024);
    }
    ...
    fn test_byte_value_serde() {
        ...
        let parsed: std::collections::HashMap<String, ByteValue> = toml::from_str(toml_str).unwrap();
        ...
        let test_struct = TestStruct { value: ByteValue(2048) };
